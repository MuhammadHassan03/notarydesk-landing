import Icon, { IconName } from "@/components/ui/icons";

export function FormSection({ title, icon, children }: {
  title: string; icon: IconName; children: React.ReactNode
}) {
  return (
    <div className="rounded-2xl p-6 mb-5" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
      <div className="flex items-center gap-2.5 mb-5 pb-3" style={{ borderBottom: '1px solid var(--divider)' }}>
        <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'var(--primary-light)' }}>
          <Icon name={icon} size={15} style={{ color: 'var(--primary)' }} />
        </div>
        <span className="text-[12px] font-bold tracking-[1.2px] uppercase" style={{ color: 'var(--text-secondary)' }}>
          {title}
        </span>
      </div>
      {children}
    </div>
  )
}